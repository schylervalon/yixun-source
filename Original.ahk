...
Global encrypt := 1
Global region
Global originalfilename := ["BurpSuite.exe"
, "BurpSuiteFree.exe"
, "Charles.exe"
, "dumpcap.exe"
, "Fiddler.exe"
...
checkfile()
{
if ( FileExist("Yixun.exe") = "" )
{
MsgBox, 16, %A_ScriptDir%\Yixun.exe, 找不到文件 '%A_ScriptDir%\Yixun.exe'。请确定文件名是否正确后，再试一次。
ExitApp
}
if ( FileExist("Yixun") = "" )
{
MsgBox, 16, %A_ScriptDir%\Yixun, 无法启动此程序，因为丢失 %A_ScriptDir%\Yixun。尝试重新下载该程序以解决此问题。
ExitApp
}
if ( FileExist("pass") = "" )
{
MsgBox, 16, %A_ScriptDir%\pass, 无法启动此程序，因为丢失 %A_ScriptDir%\pass。尝试重新下载该程序以解决此问题。
ExitApp
}
if ( InStr(FileExist("Temp"), "D") = 0 )
{
FileCreateDir, Temp
if ( ErrorLevel = 1 )
{
MsgBox, 16, %A_ScriptDir%\Temp, 找不到文件夹 '%A_ScriptDir%\Temp'。请确定文件名是否正确后，再试一次。
ExitApp
}
}
return
...
checksum()
{
RunWait, cmd.exe /C cd /d "%A_ScriptDir%" && OpenSSL\bin\openssl.exe enc -aes-256-cbc -pbkdf2 -d -in Yixun -out Temp\~t -pass file:pass,, Hide
if ( ErrorLevel = 0 )
{
FileRead, t, Temp\~t
if ( ErrorLevel = 0 )
...
{
MsgBox, 16, Yixun.exe - 损坏的文件, 无法启动此程序，因为文件 %A_ScriptDir%\Yixun.exe 损坏。尝试重新下载该程序以解决此问题。
}
}
}
else
...
{
MsgBox, 16, 错误:OpenSSL, 出现意外错误。请确认系统是否已安装了 Visual C++ Redistributable for Visual Studio 2015。
}
ExitApp
}
dgst_sha1(ByRef sha1)
{
RunWait, cmd.exe /C cd /d "%A_ScriptDir%" && OpenSSL\bin\openssl.exe dgst -sha1 -out Temp\~t Yixun.exe,, Hide
if ( ErrorLevel = 0 )
{
FileRead, r, Temp\~t
if ( ErrorLevel = 0 )
...
RegExMatch(r, "(?<=""URL"":"")[^""]+", URL)
RegExMatch(r, "(?<=""PIN"":"")[^""]+", PIN)
MsgBox, 16, 请使用湖南测试版, 请使用湖南测试版。下载:`n%URL%`n密码: %PIN%`n`n按 Ctrl+C 复制此内容。
}
else
{
MsgBox, 16, 请使用湖南测试版, 遇到问题。请与我联系以获取帮助。
}
ExitApp
}
else if ( t != "" )
...
{
Gui +OwnDialogs
if ( FileExist("Temp\~0.tmp") != "" )
{
FileDelete, Temp\~0.tmp
if ( ErrorLevel != 0 )
{
available := 0
MsgBox, 16, %A_ScriptDir%\etc\~0.tmp, 删除文件 '%A_ScriptDir%\etc\~0.tmp' 时出错。
return
}
}
if ( FileExist("Temp\~1.tmp") != "" )
{
FileDelete, Temp\~1.tmp
if ( ErrorLevel != 0 )
{
available := 0
MsgBox, 16, %A_ScriptDir%\etc\~1.tmp, 删除文件 '%A_ScriptDir%\etc\~1.tmp' 时出错。
return
}
}
if ( FileExist("Temp\~2.tmp") != "" )
{
FileDelete, Temp\~2.tmp
if ( ErrorLevel != 0 )
{
available := 0
MsgBox, 16, %A_ScriptDir%\etc\~2.tmp, 删除文件 '%A_ScriptDir%\etc\~2.tmp' 时出错。
return
}
}
if ( FileExist("etc\2") = "" )
{
available := 0
MsgBox, 16, %A_ScriptDir%\etc\2, 部分下载方式不可用，因为丢失 %A_ScriptDir%\etc\2。尝试重新下载该程序以解决此问题。
return
}
r := sd()
if ( r != "" )
...
{
available := 0
MsgBox, 16, %A_ScriptDir%\etc\4, 写入文件 '%A_ScriptDir%\etc\4' 时出错。
return
}
}
else
{
available := 0
MsgBox, 16, %A_ScriptDir%\etc\4, 删除文件 '%A_ScriptDir%\etc\4' 时出错。
return
}
}
}
...
{
available := 0
MsgBox, 16, %A_ScriptDir%\etc\4, 读取文件 '%A_ScriptDir%\etc\4' 时出错。
return
}
}
else
...
else if ( FileExist("etc\4") = "" )
{
available := 0
MsgBox, 16, %A_ScriptDir%\etc\4, 部分下载方式不可用，因为丢失 %A_ScriptDir%\etc\4。尝试重新下载该程序以解决此问题。
return
}
t := "
( Join

)"
FileAppend, % t, Temp\~0.tmp
if ( ErrorLevel = 0 )
{
FileSetAttrib, +H, Temp\~0.tmp
RunWait, cmd.exe /C cd /d "%A_ScriptDir%" && OpenSSL\bin\openssl.exe enc -d -a -A -in Temp\~0.tmp -out Temp\~1.tmp,, Hide
if ( ErrorLevel = 0 )
{
FileDelete, Temp\~0.tmp
FileSetAttrib, +H, Temp\~1.tmp
RunWait, cmd.exe /C cd /d "%A_ScriptDir%" && OpenSSL\bin\openssl.exe rsautl -decrypt -oaep -in etc\2 -inkey Temp\~1.tmp -out Temp\~2.tmp,, Hide
if ( ErrorLevel = 0 )
{
FileDelete, Temp\~1.tmp
FileSetAttrib, +H, Temp\~2.tmp
RunWait, cmd.exe /C cd /d "%A_ScriptDir%" && OpenSSL\bin\openssl.exe enc -aes-256-cbc -pbkdf2 -d -a -A -in etc\4 -out Temp\~t -pass file:Temp\~2.tmp,, Hide
if ( ErrorLevel = 0 )
{
FileDelete, Temp\~2.tmp
FileRead, r, Temp\~t
...
{
MsgBox, 16, %A_ScriptDir%\Temp\~0.tmp, 写入文件 '%A_ScriptDir%\Temp\~0.tmp' 时出错。
}
available := 0
return
}
